...
function setParam() {
	...
	function doSearch(e) {
		/* var check_sdate = $('#check_s_year').val()+""+$('#check_s_month').val();
		$('#check_sdate').val(check_sdate);
		var check_edate = $('#check_e_year').val()+""+$('#check_e_month').val();
		$('#check_edate').val(check_edate); */
		$('#page').val(1);
		$('#frm').submit();
	}
	...
	function doDetail(subject_seqno,project_seqno,tsc_check_date){
		$('#subject_seqno').val(subject_seqno);
		$('#project_seqno').val(project_seqno);
		$('#tsc_check_date').val(tsc_check_date);
		$('#frm2').submit();
	}
</script>
<form action="/subject/checkDetail.do" id="frm2" name="frm2" method="post">
	<input type="hidden" name="subject_seqno" id="subject_seqno" value="" />
	<input type="hidden" name="project_seqno" id="project_seqno" value="" />
	<input type="hidden" name="tsc_check_date" id="tsc_check_date" value="" />
</form>
						...
						<div class="search_box_inner">
							...
							<div class="search_cu_box">
								<span>진행상태</span> <select class="select_normal select_status" style="width:393px;"
									...
									<option title="전체 (점검완료 제외)" value="">전체 (점검완료 제외)</option>
									<option title="제출대상" value="00">제출대상</option>
	<!-- 								<option value="10">작성</option>
	 -->								<option title="자료제출" value="20">자료제출</option>
		<!-- 							<option title="점검" value="30">점검</option>
		 -->							<option title="점검완료" value="50">점검완료</option>
									<option title="보완대상" value="80">보완대상</option>
									<option title="현장실태조사" value="85">현장실태조사</option>
<!-- 									<option value="90">점검거부</option>
 -->								</select>
							</div><br/>
							<div class="search_cu_box b_name_box">
								<span>점검년월</span> 
								<input type="text" name="check_sdate" id="check_sdate" placeholder="점검시작년월" value="${paraMap.check_sdate }" class="only_ym"  title="점검시작년월">
								<input type="text" name="check_edate" id="check_edate" placeholder="점검종료년월" value="${paraMap.check_edate }" class="only_ym" title="점검종료년월">
							</div>
							<div class="btn_wrap">
								<button type="button" class="btn_step"  onClick="javascript:doSearch();" style="margin-left:257px;">
									<span>검색</span>
								</button>
							</div>
						</div>
					...
					<div class="subject_corp">
						<h3 class="tbl_ttc">
							진도점검 목록  <span class="total_cnt">총 ${paraMap.count} 건</span>
						</h3>
					...
					<div class="tbl_comm tbl_public">
						<table class="tbl">
							<caption class="caption_hide">진도점검 리스트</caption>
							<colgroup>
							...
							<tbody>
								<c:forEach var="data" items="${result }" varStatus="status">
									<tr>
										<td>${status.count}</td>
										<td>${fn:substring(data.tsc_check_date,0,4)}-${fn:substring(data.tsc_check_date,4,6)}</td>
										<td>${data.tsc_check_no }</td>
										...
										<td>
											<c:if test="${data.tsc_subject_proc_status  eq '00' }">
												제출대상
											</c:if>
<%-- 											<c:if test="${data.tsc_subject_proc_status  eq '10' }">
												작성
											</c:if> --%>
											<c:if test="${data.tsc_subject_proc_status  eq '20' }">
												자료제출
											</c:if>
<%-- 											<c:if test="${data.tsc_subject_proc_status  eq '30' }">
												점검
											</c:if>
 --%>											<c:if test="${data.tsc_subject_proc_status  eq '50' }">
												점검완료
											</c:if>
											<c:if test="${data.tsc_subject_proc_status  eq '80' }">
												보완대상
											</c:if>
											<c:if test="${data.tsc_subject_proc_status  eq '85' }">
												현장실태조사
											</c:if>
<%-- 											<c:if test="${data.tsc_subject_proc_status  eq '90' }">
												점검거부
											</c:if> --%>
										</td>
										<td>
											<a href="javascript:void(0);" onclick="doDetail('${data.subject_seqno}','${data.project_seqno}' ,'${data.tsc_check_date}');" title="진도점검 상세조회">[상세조회]</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
