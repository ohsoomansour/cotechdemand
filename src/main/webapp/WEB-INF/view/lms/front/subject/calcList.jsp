...
<form action="/subject/calcDetail.do" id="frm2" name="frm2" method="post">
						...
						<div class="search_box_inner">
							...
							<div class="search_cu_box">
								<span>진행상태</span> <select class="select_normal select_status" style="width:393px;"
									...
									<option title="전체(정산완료제외)" value="">전체 (정산완료제외)</option>
									<option title="제출대상" value="00">제출대상</option>
<!-- 									<option value="10">작성</option>
 -->									<option value="20">정산대기</option>
...
<!-- 									<option value="90">거부</option>
 -->								</select>
					...
					<div class="subject_corp">
						<h3 class="tbl_ttc">
							과제별 사업비 정산 현황&nbsp;<span class="total_cnt">총 ${paraMap.count} 건</span>
						</h3>
					...
					<div class="tbl_comm tbl_public">
						<table class="tbl">
							<caption class="caption_hide">사업비 정산 리스트</caption>
							<colgroup>
							...
							<tbody>
								<c:forEach var="data" items="${result }" varStatus="status">
									...
									<%-- 	<td>
										...
										<td>
											<c:if test="${data.tscc_subject_proc_status eq  '00'}">
												제출대상
											</c:if>
<%-- 											<c:if test="${data.tscc_subject_proc_status eq  '10'}">
												작성
											</c:if>
 --%>											<c:if test="${data.tscc_subject_proc_status eq  '20'}">
												정산대기
											</c:if>
<%-- 											<c:if test="${data.tscc_subject_proc_status eq  '30'}">
												검토
											</c:if>
 --%>											<c:if test="${data.tscc_subject_proc_status eq  '50'}">
												정산완료
											</c:if>
											<c:if test="${data.tscc_subject_proc_status eq  '80'}">
												보완대상
											</c:if>
<%-- 											<c:if test="${data.tscc_subject_proc_status eq  '90'}">
												거부
											</c:if>
 --%>										</td>
										<td>
											<a href="javascript:void(0);" onclick="doDetail('${data.subject_seqno}','${data.project_seqno}');" title="사업비 정산 상세조회" >[상세조회]</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
