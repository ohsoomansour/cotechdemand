...
<form action="/subject/evalDetail.do" id="frm2" name="frm2" method="post">
						...
						<div class="search_box_inner">
							...
							<div class="search_cu_box">
								<span>진행상태</span> <select class="select_normal select_status" style="width:393px;"
									...
									<option title="전체 (최종평가완료 제외)" value="">전체 (최종평가완료 제외)</option>
									<option title="제출대상" value="00">제출대상</option>
<!-- 									<option value="10">작성</option>
-->									<option title="최종평가대기" value="20">최종평가대기</option>
<!-- 									<option value="30">검토</option>
-->									<option title="최종평가완료" value="50">최종평가완료</option>
<!-- 									<option value="59">평가완료(불합격)</option>
-->									<option title="보완대상" value="80">보완대상</option>
<!-- 									<option value="90">평가거부</option>
 -->								</select>
							</div>
							<div class="btn_wrap">
					...
					<div class="subject_corp">
						<h3 class="tbl_ttc">
							최종평가 목록&nbsp;<span class="total_cnt">총 ${paraMap.count} 건</span>
						</h3>
					...
					<div class="tbl_comm tbl_public">
						<table class="tbl">
							<caption class="caption_hide">최종평가 리스트</caption>
							<colgroup>
...
<%-- 										<td>
										...
										<td>
											<c:if test="${data.tse_subject_proc_status  eq '00' }">
												제출대상
											</c:if>
<%-- 											<c:if test="${data.tse_subject_proc_status  eq '10' }">
												작성
											</c:if>
 --%>											<c:if test="${data.tse_subject_proc_status  eq '20' }">
												최종평가대기
											</c:if>
<%-- 											<c:if test="${data.tse_subject_proc_status  eq '30' }">
												검토
											</c:if>
 --%>											<c:if test="${data.tse_subject_proc_status  eq '50' }">
												최종평가완료
											</c:if>
<%-- 											<c:if test="${data.tse_subject_proc_status  eq '59' }">
												평가완료(불합격)
											</c:if>
 --%>											<c:if test="${data.tse_subject_proc_status  eq '80' }">
												보완대상
											</c:if>
<%-- 											<c:if test="${data.tse_subject_proc_status  eq '90' }">
												평가거부
											</c:if>
 --%>										</td>
										<td>
											<a href="javascript:void(0);" onclick="doDetail('${data.subject_seqno}','${data.project_seqno}' );" title="최종평가 상세조회 ">[상세조회]</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
