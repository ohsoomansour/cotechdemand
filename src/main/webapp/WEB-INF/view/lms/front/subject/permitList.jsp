...
<form action="/subject/permitDetail.do" id="frm2" name="frm2" method="post">
	...
	<div id="compaVcContent" class="cont_cv">
		<div id="mArticle" class="assig_app">
			...
			<div class="wrap_cont">
				<!-- page_title s:  -->
				<div class="area_tit">
					<h3 class="tit_corp">사업수행관리 - 권한부여</h3>
				</div>
				<!-- //page_title e:  -->
				<!-- page_content s:  -->
				<div class="area_cont">
						...
						<div class="search_box_inner">
							...
							<div class="search_cu_box">
								<span>진행상태</span> <select class="select_normal select_status" style="width:393px;"
									...
									<option title="권한부여" value="20">권한부여</option>
									<option title="사용확인" value="50">사용확인</option>
									<option title="사용거부" value="90">사용거부</option>
					...
					<div class="subject_corp">
						<h3 class="tbl_ttc">
							권한부여 현황  <span class="total_cnt">${paraMap.count} 건</span>
						</h3>
					...
					<div class="tbl_comm tbl_public">
						<table class="tbl">
							<caption class="caption_hide">권한부여 리스트</caption>
							<colgroup>
							...
							<tbody>
								<c:forEach var="data" items="${result }" varStatus="status">
									<tr>
										...
										<td>
											<c:if test="${data.tsp_admin_date eq '1999-01-01 00:00:00'}" >
												미완료
											</c:if>
											<c:if test="${data.tsp_admin_date ne '1999-01-01 00:00:00'}" >
												${fn:substring(data.tsp_admin_date,0,11)}
											</c:if>
											
										</td>
										<td>
											<c:if test="${data.tsp_subject_proc_status  eq '00' }">
												준비
											</c:if>
											<c:if test="${data.tsp_subject_proc_status  eq '20' }">
												권한부여
											</c:if>
											<c:if test="${data.tsp_subject_proc_status  eq '50' }">
												사용확인
											</c:if>
											<c:if test="${data.tsp_subject_proc_status  eq '90' }">
												사용거부
											</c:if>
											<c:if test="${data.tsp_subject_proc_status  eq '99' }">
												취소
											</c:if>
										</td>
										<td>
											<a href="javascript:void(0);" onclick="doDetail('${data.subject_seqno}','${data.project_seqno}' );" title="권한부여 상세조회">[상세조회]</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
