...
window.onload = function(){
	...
	console.log(list);
	 var options = {
    	fmstSeq:   ${paraMap.calcdata.fmst_seqno},
    	//fmstSeq: 11,
        types: list,
        callback: function(data){
	    	...
	    	doCalcSubmit(status,data);
		},
    };
    window.uploadSrc.create($('#uploadArea'), options);	    	
    var subject_status = "${paraMap.calcdata.subject_proc_status }";
...
function validataCheck(stat){
	...
	var check = confirm("[정산관리 자료]를 제출하시겠습니까? ")
		if (check == true) {
			uploadFile(stat)
			//doPermitSubmit();
		}
...
function doCalcSubmit(status,fmst_seqno){
	var url = "/subject/calcSubmit.do"
		var form = $('#frm')[0];
...
function goList(){
	window.location.href="/subject/calcList.do";
}
</script>
<form id="frm" name="frm" action ="/subject/calcSubmit.do" method="post" >
<div id="compaVcContent" class="cont_cv">
	<div id="mArticle" class="assig_app">
		...
		<div class="wrap_cont">
			<!-- page_title s:  -->
			<div class="area_tit">
				<h3 class="tit_corp">사업비 정상 상세정보</h3>
			</div>
			<!-- //page_title e:  -->
			<!-- page_content s:  -->
			<div class="area_cont">
			 ...
			 <div class="wrap_btn _center">
			 	<a href="/subject/calcList.do"  class="btn_appl">목록으로 이동</a>
				<a href="javascript:void(0);" onclick="validataCheck('submit')" class="btn_appl">[정산관리  자료제출]</a>
			</div> 
			</div>
			<!-- //page_content e:  -->
		</div>
